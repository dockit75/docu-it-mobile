    ...
    if (phone?.length >= 10) {
      if (value?.length === 4) {
        ...
        try {
          ...
          let message = error?.response?.data?.message === 'Invalid credential' ? 'Invalid PIN. Please try again. ' : error.response ? error?.response?.data?.message : 'Network Error. Please check your internet connection'
